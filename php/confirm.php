...
$orderStmt = $conn->prepare("INSERT INTO orders (user_id, fullname, address, email, card_last4, total, created_at, order_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
$orderStmt->bind_param("isssssss", $user_id, $fullname, $address, $email, $card_last4, $total, $order_date, $order_date);
$orderStmt->execute();
...
foreach ($cartData as $item) {
  ...
  $itemStmt->bind_param("isid", $order_id, $productName, $qty, $price);
  $itemStmt->execute();
  $itemStmt->close();
}
?>
